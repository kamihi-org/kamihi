...
@pytest.mark.asyncio
async def test_create_user(admin_page: Page, credentials):
    ...
    await admin_page.get_by_role("spinbutton", name="Telegram id*").fill(str(credentials.user_id))
    ...
    await expect(admin_page.locator("tbody")).to_contain_text(str(credentials.user_id))
