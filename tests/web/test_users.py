...
async def test_create_user(admin_page: Page, test_settings):
    ...
    await admin_page.get_by_role("spinbutton", name="Telegram id*").fill(str(test_settings.credentials.user_id))
    ...
    await expect(admin_page.locator("tbody")).to_contain_text(str(test_settings.credentials.user_id))
