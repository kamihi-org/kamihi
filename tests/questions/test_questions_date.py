"""
Functional tests for the date type question.
...
async def test_base(user, add_permission_for_user, chat: Conversation):
    """Test the base functionality of the date question with a full datetime input."""
    add_permission_for_user(user["telegram_id"], "start")

    await chat.send_message("/start")
    response = await chat.get_response()
    assert "Please provide a date." in response.text

    await chat.send_message("2020-01-01 12:34")
    final_response = await chat.get_response()
    assert "Your date is 2020-01-01." in final_response.text


@pytest.mark.asyncio
@pytest.mark.parametrize(
    "question_args,valid_input,expected_date,invalid_input",
    [
        (
...
async def test_validation(
    ...
    """Test validation of the date question with constraints and natural language."""
    add_permission_for_user(user["telegram_id"], "start")
    ...
    final_response = await chat.get_response()
    assert f"Your date is {expected_date}." in final_response.text
